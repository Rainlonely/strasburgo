...
function optionsframework_admin_init() {
		...
		global $of_options_munditia_pmc, $options_machine;




		$options_machine = new Options_Machine($of_options_munditia_pmc);
...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
    <div id="header">
      <div class="logo">
        <h2>Munditia - Premium Wordpress Theme</h2> 
      </div>
	...
	foreach ($options as $value) {
		...
		case 'slider':
				...
				if ($count < 2) {
					$oldorder = 1;
					$order = 1;
					$output .= Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int,$value['descshow'],$value['team'],$value['nivo'],$value['social']);
				} else {
					$i = 0;
				...
				$output .= '</ul>';
				if($value['descshow'] and !$value['team'] and !$value['nivo'] and !$value['social']){
				$output .= '<a href="#" class="button slide_add_button">Add New Slide</a></div>';
				}
				if($value['team'] and !$value['descshow'] and !$value['nivo'] and !$value['social']){
		...
		case 'heading':
			...
			$menu .= '<li class="fa '. $header_class .'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'">'.  $value['name'] .'</a></li>';
...
public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int,$desc, $team = false, $nivo = false, $social = false){
	...
	$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. str_replace('\\','',$val['title']) .'" />';	
	$slider .= '<label>Social Icon custom class</label>';
	if (!empty($val['class'])) {
		$slider .= '<input class="slide of-input of-slider-class" name="'. $id .'['.$order.'][class]" id="'. $id .'_'.$order .'_slide_class" value="'. str_replace('\\','',$val['class']) .'" />';	
	}	
	}
	else{
